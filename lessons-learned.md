...
### [Python / Claude API / SQLAlchemy] — Claude top-level response field silently dropped when saving nested dict

**Date:** 2026-04-01
**Problem:** `report --detail` showed `Positioning: —` for all 5 Gumroad suggestions even though Claude was returning a `positioning_note` field in its enrichment response.
**Root cause:** `enrichment_engine.py` saved `marketing_json = json.dumps(parsed['marketing'])` — only the nested `marketing` object. Claude returns `positioning_note` at the top level of the response, not nested inside `marketing`. It was never written to DB.
**Fix:** Merged `positioning_note` into the marketing dict before serializing: `marketing['positioning_note'] = parsed['positioning_note']`. Then ran a backfill script to update all existing suggestion rows from the already-stored `suggestion_enrichment` analyses in the DB.
**Watch out for:** Any time Claude's response schema has fields at multiple nesting levels, check that every top-level field is explicitly handled — Python dict access on a sub-key silently ignores sibling keys at the parent level.
